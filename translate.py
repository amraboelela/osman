...
for line in lines:
    if "-->" in line:
        if len(paragraph) > 0:
            ...
            data = r.json()
            translatedText = data['data']['translations'][0]['translatedText'].replace('-','').replace("&#39;","'").replace("&quot;","'").replace("'il","'ll").replace(" gas "," conquest ").replace(" gas."," conquest.").replace("Come on God","Allah is Ever-Living").replace("Sorry.","Sir.").replace(" oba "," tribe ").capitalize()
            try:
